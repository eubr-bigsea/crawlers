...
def get_parameters():
    ...
    parser = argparse.ArgumentParser(description='Get data about the \
        Weather in regions from the Open Weather API')
...
def get_data(regions, API_key):
    ...
    for region in regions:
        print region
        # REQUESTS INFORMATION ABOUT THE WEATHER IN THE CENTROIDS AND INSERT TO THE MONGO DB
        link = "http://api.openweathermap.org/data/2.5/weather?lat=" + region['latitude'] + "&lon=" + region['longitude'] + "&APPID=" + API_key
